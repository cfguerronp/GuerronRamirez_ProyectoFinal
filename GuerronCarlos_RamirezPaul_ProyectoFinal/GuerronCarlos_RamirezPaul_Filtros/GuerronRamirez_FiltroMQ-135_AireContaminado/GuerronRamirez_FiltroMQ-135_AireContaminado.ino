/*
 *      Filtro del sensor mq-135
 * Se filtra los datos que se obtuvieron en la medicion de lso sensores
 * 
 */
...
void loop() {
  gauss_filtro ((double *)&mq_135[0], (double *)&output[0], (int) tam_sig,5); //llamar al metodo gauss
...
void plot_signal(void){ //metodo para graficar la señal
  ...
  for(i=0;i<tam_sig;i++){//ciclo repetitivo para recorrer la señal
      Serial.print(mq_135[i]);//posiscion del dato
      Serial.print(",");
